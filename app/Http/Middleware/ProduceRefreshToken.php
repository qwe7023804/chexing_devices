...
class ProduceRefreshToken extends BaseMiddleware
{
    /**
    ...
    public function handle($request, Closure $next)
    {
        ...
        $authToken = auth('produce')->getToken();
        if (!$authToken) {
            return response()->json(['code' => 401, 'msg' => '未登录']);
        }

        // 检测用户的登录状态，如果正常则通过
        if (auth('produce')->check()) {
            $admin_id = Auth::guard('produce')->payload()['sub'];
            $time = auth('produce')->payload()['exp'];

            //刷新Token
            if (($time - time()) < 10 * 60 && ($time - time()) > 0) {
                $token = auth('produce')->refresh();
                if ($token) {
