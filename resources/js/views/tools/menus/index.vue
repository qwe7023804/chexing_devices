<template>
    <div>
        ...
        <!--目录排序-->
        <directory-sort ref="chilSortMenu"></directory-sort>
        <!--修改菜单信息-->
        <edit-menu ref="chilEditMenu"></edit-menu>
    </div>
</template>

<script>
    import AddMenu from "@/components/Tools/Menus/AddMenu";
    import DirectorySort from "@/components/Tools/Menus/DirectorySort";
    import EditMenu from "@/components/Tools/Menus/EditMenu";
    ...
    export default {
        ...
        methods: {
            get_menus_list() {
                let _this = this;
                this.$get(this.$tools.getMenusList).then((res) => {
            ...
            directoryDelete(id) {
                var _this = this;
                this.$delete(this.$tools.delMenus + id).then((res) => {
                    if (res.code === 200){
                        _this.$message({message: '删除！', type: 'success'});
                        _this.reload();
                    }else {
