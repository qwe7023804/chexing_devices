...
axios.interceptors.response.use(function (res) {
    ...
    if(res.data.code == 401){
        ...
        let reSp110 = /^\/sp110\/+./i
        let reTools = /^\/tools\/+./i
        let reProduce = /^\/produce\/+./i

        let fullepath = router.history.current.fullPath;

        // if(reSp110.test(fullepath)){  // 后台就跳转后台
        //     router.push('/sp110/login');
        // }else if(reTools.test(fullepath)){  // 商家跳转
        //     router.push('/tools/login');
        // }
        if(reSp110.test(fullepath)){  // 后台就跳转后台
            router.push('/sp110/login');
        }
        if(reTools.test(fullepath)){  // 商家跳转
            router.push('/tools/login');
        }

        if (reProduce.test(fullepath)){
            router.push('/produce/login')
        }
    }
